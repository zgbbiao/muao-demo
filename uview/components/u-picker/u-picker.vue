<template>
	<u-popup :maskCloseAble="maskCloseAble" mode="bottom" :show="show" length="auto" :safeAreaInsetBottom="safeAreaInsetBottom"  @close="getResult()">
		<view class="u-datetime-picker" @tap.stop>
			<view class="u-picker-header" @touchmove.stop.prevent="stop" catchtouchmove="stop">
				...
				<view class="u-btn-picker u-type-primary" :style="{ color: confirmColor }" hover-class="u-type-success"
				 :hover-stay-time="150" @tap="getResult('confirm')">确定</view>
			</view>
			<view class="u-picker-body">
				<picker-view v-if="mode == 'region'" :value="value" @change="change" class="u-picker-view">
				...
				<picker-view v-else :value="value" @change="change" class="u-picker-view">
...
<script>
	...
	export default {
		...
		watch: {
			...
			// 微信小程序由于一些奇怪的原因，需要重新初始化才能显示正确的值
			show() {
				// #ifdef MP-WEIXIN
				this.reset = true;
				setTimeout(() => this.init(), 10);
		...
		methods: {
			...
			setYears() {
				...
				// 设置this.value某一项的值，是为了让picker预选中某一个值
				this.value.splice(this.value.length - 1, 1, this.getIndex(this.years, this.year));
			},
			setMonths() {
				this.months = this.generateArray(1, 12);
				this.value.splice(this.value.length - 1, 1, this.getIndex(this.months, this.month));
			},
			setDays() {
				let totalDays = new Date(this.year, this.month, 0).getDate();
				this.days = this.generateArray(1, totalDays);
				let index = 0;
				// 这里不能使用类似setMonths()中的this.value.splice(this.value.length - 1, xxx)做法
				// 因为this.month和this.year变化时，会触发watch中的this.setDays()，导致this.value.length计算有误
				if(this.params.year && this.params.month) index = 2;
				else if(this.params.month) index = 1;
				else if(this.params.year) index = 1;
				else index = 0;
				this.value.splice(index, 1, this.getIndex(this.days, this.day));
			},
			setHours() {
				this.hours = this.generateArray(0, 23);
				this.value.splice(this.value.length - 1, 1, this.getIndex(this.hours, this.hour));
			},
			setMinutes() {
				this.minutes = this.generateArray(0, 59);
				this.value.splice(this.value.length - 1, 1, this.getIndex(this.minutes, this.minute));
			},
			setSeconds() {
			...
			setProvinces() {
				...
				// 设置默认省份的值
				this.value.splice(0, 1, this.province);
			},
			setCitys() {
				...
				this.city = tmp;
				this.citys = citys[this.province];
				this.value.splice(1, 1, this.city);
			},
			setAreas() {
				...
				this.area = tmp;
				this.areas = areas[this.province][this.city];
				this.value.splice(2, 1, this.area);
			},
			close() {
			...
			change(e) {
				...
				let i = 0;
				if (this.mode == 'time') {
					// 这里使用i++，是因为this.value数组的长度是不确定长度的，它根据this.params的值来配置长度
					// 进入if规则，i会加1，保证了能获取准确的值
					if (this.params.year) this.year = this.years[this.value[i++]];
					if (this.params.month) this.month = this.months[this.value[i++]];
					if (this.params.day) this.day = this.days[this.value[i++]];
					if (this.params.hour) this.hour = this.hours[this.value[i++]];
					if (this.params.minute) this.minute = this.minutes[this.value[i++]];
					if (this.params.second) this.second = this.seconds[this.value[i++]];
				} else {
					if (this.params.province) this.province = this.value[i++];
					if (this.params.city) this.city = this.value[i++];
					if (this.params.area) this.area = this.value[i++];
				}
			},
			// 用户点击确定按钮
